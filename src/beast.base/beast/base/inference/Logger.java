...
public class Logger extends BEASTObject {
    ...
    public void initAndValidate() {
        ...
        if (mode.equals(LOGMODE.autodetect)) {
            ...
            // hack to get around dependency on beast.evolution.Tree
            if (loggerCount == 1 &&             		
            		loggerList.get(0) instanceof StateNode &&
            		loggerList.get(0).getClass().getName().contains("Tree")) { // instanceof Tree) {
    ...
    public void init() throws IOException {
        ...
        if (needsHeader) {
            if (modelInput.get() != null) {
            	...
            	// hack to get around dependency on XMLParser, which depends on beast.evolution
                String xml = ModelLogger.modelToString(modelInput.get()); 
                xml = "#" + xml.replaceAll("\\n", "\n#");
                m_out.println("#\n#model:\n#");
                m_out.println(xml);
                m_out.println("#");
	...
	public void setLogOffset(long offset) throws IOException {
	    final File file = new File(fileName);
	    if (file.exists()) {
	    	// back up file in case something goes wrong (e.g. an out of memory error occurs)
	        final File treeFileBackup = new File(fileName);
