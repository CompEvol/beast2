...
public class BeautiPanelConfig extends Plugin {
	...
	public void initAndValidate() throws Exception {
		...
		startInputs = new ArrayList<Plugin>();
	...
	public Input<?> resolveInput(BeautiDoc doc, int iPartition) {
		try {
			...
			parentPlugins = new ArrayList<Plugin>();
...
//							    (m_sConditionalAttribute[i].equals("type") && plugin.getClass().getName().equals(m_sConditionalValue[i]))) {
			...
			if (!bIsList) {
				_input = new FlexibleInput<Plugin>();
			} else {
				_input = new FlexibleInput<ArrayList<Plugin>>(new ArrayList<Plugin>());
			}
			_input.setRule(Validate.REQUIRED);
			syncTo(iPartition);
			if (parentPlugins != null && parentPlugins.size() > 0) System.err.println("sync " + parentPlugins.get(iPartition) + "[?] = " + _input.get());
