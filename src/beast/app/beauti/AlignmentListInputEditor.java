...
System.err.println("needsRePartition = " + needsRePartition);			
			if (needsRePartition) {
				...
				if (tModels.size() == 1) {
					...
					BEASTObject.setInputValue(((BEASTInterface)oldTree), "estimate", false);
                	// use toArray to prevent ConcurrentModificationException
					for (Object plugin : BEASTObject.getOutputs(oldTree).toArray()) { //.toArray(new BEASTInterface[0])) {
						for (Input<?> input : BEASTObject.listInputs((BEASTInterface) plugin)) {
							try {
