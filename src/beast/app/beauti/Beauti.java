...
//			}
    ...
    void setMenuVisibility(String sParentName, Component c) {
        String sName = "";
        ...
        if (c instanceof JMenu) {
            for (Component x : ((JMenu) c).getMenuComponents()) {
                setMenuVisibility(sParentName + sName + (sName.length() > 0 ? "." : ""), x);
            }
        } else if (c instanceof Container) {
            for (int i = 0; i < ((Container) c).getComponentCount(); i++) {
                setMenuVisibility(sParentName, ((Container) c).getComponent(i));
            }
        }
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            // Make things look nice on Mac OS X
            System.setProperty("com.apple.macos.useScreenMenuBar", "true");
            System.setProperty("com.apple.mrj.application.apple.menu.about.name", "Beauti II");
            ...
            BeautiInitDlg dlg = new BeautiInitDlg(args, doc);
