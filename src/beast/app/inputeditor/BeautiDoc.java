...
import beast.parser.XMLParser.RequiredInputProvider;
...
public class BeautiDoc extends BEASTObject implements RequiredInputProvider {
    ...
    final public Input<beast.inference.Runnable> mcmc = new Input<>("runnable", "main entry of analysis",
            Validate.REQUIRED);
    ...
    // partition names
    public List<PartitionContext> partitionNames = new ArrayList<>();
    ...
    public Map<String, String> tipTextMap = new HashMap<>();
     ...
     */
    public List<BEASTInterface> posteriorPredecessors = null;
    ...
    public InputEditorFactory inputEditorFactory;
...
//      fireDocHasChanged();
    ...
    public void initialize(ActionOnExit endState, String xml, String template, String fileName) throws XMLParserException, SAXException, IOException, ParserConfigurationException {
        // beautiConfig.clear();
        switch (endState) {
            case UNKNOWN:
...
//		}
    ...
    public void extractSequences(String xml) throws XMLParserException, SAXException, IOException, ParserConfigurationException  {
        ...
        beauti.autoSetClockRate(autoSetClockRate);
        allowLinking = beautiStatus.contains("allowLinking");
        beauti.allowLinking(allowLinking);
        autoUpdateFixMeanSubstRate = !beautiStatus.contains("noAutoUpdateFixMeanSubstRate");
        beauti.autoUpdateFixMeanSubstRate(autoUpdateFixMeanSubstRate);
        ...
        try {
            CompoundDistribution posterior = (CompoundDistribution) ((beast.inference.MCMC) mcmc.get()).posteriorInput.get();
                ...
                if (distr.getID().equals("likelihood")) {
                    for (Distribution likelihood : ((CompoundDistribution) distr).pDistributions.get()) {
                        if (likelihood instanceof GenericTreeLikelihood) {
                            ...
                            PartitionContext context = PartitionContextUtil.newPartitionContext(treeLikelihood);
                            try {
...
//		}
    ...
    public void loadTemplate(String xml) throws XMLParserException, SAXException, IOException, ParserConfigurationException  {
        ...
        for (BEASTInterface beastObject : beastObjects) {
            if (beastObject instanceof beast.inference.Runnable) {
     ...
     */
    public void addTraitSet(TraitSet trait) {
        if (trait != null) {
...
//                            "Cannot link all clock model(s) except strict clock with different trees !");
    ...
    public BranchRateModel getClockModel(String partition) {
        int k = 0;
        for (Alignment data : alignments) {
            if (data.getID().equals(partition)) {
...
//        str = str.replaceAll("\\$\\(n\\)", partition.partition);
    ...
    public int getPartitionNr(String partition, int partitionID) {
    ...
    public int getPartitionNr(BEASTInterface beastObject) {
    ...
    public void addLink(BEASTInterface from, BEASTInterface to) {
        try {
