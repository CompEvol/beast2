...
public class XMLProducer extends XMLParser {
	...
	void pluginToXML(Plugin plugin, StringBuffer buf, String sName) throws Exception {
		...
		if (plugin instanceof Data) {sElementName=XMLParser.DATA_ELEMENT;}
		if (plugin instanceof Sequence) {sElementName=XMLParser.SEQUENCE_ELEMENT;}
		if (plugin instanceof State) {sElementName=XMLParser.STATE_ELEMENT;}
		if (plugin instanceof Uncertainty) {sElementName=XMLParser.LIKELIHOOD_ELEMENT;}
		if (plugin instanceof Logger) {sElementName=XMLParser.LOG_ELEMENT;}
		if (plugin instanceof MCMC) {sElementName=XMLParser.MCMC_ELEMENT;}
		if (plugin instanceof Operator) {sElementName=XMLParser.OPERATOR_ELEMENT;}
		if (plugin instanceof Parameter) {sElementName=XMLParser.PARAMETER_ELEMENT;}
		if (plugin instanceof Tree) {sElementName=XMLParser.TREE_ELEMENT;}
