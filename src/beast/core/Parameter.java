...
@Description("A parameter represents a value in the state space that can be changed " +
		"by operators.")
public class Parameter extends Plugin {
    ...
    public int getDimension() {return m_values.length;}
    public boolean hasID(String sID) {return m_sID.equals(sID);}
    public double getValue() {return m_values[0];}
    public int intValue() {return (int)m_values[0];}
    public boolean boolValue() {return (m_values[0] != 0.0);}

    public double getLower() {return m_fLower;}
    public void setLower(double fLower) {m_fLower = fLower;}
    public double getUpper() {return m_fUpper;}
    public void setUpper(double fUpper) {m_fUpper = fUpper;}
    public double getValue(int iParam) {return m_values[iParam];}
    public void setBounds(double fLower, double fUpper) {m_fLower = fLower; m_fUpper = fUpper;}

	public void setValue(double fValue) {
		m_values[0] = fValue;
		m_bIsDirty = State.IS_DIRTY;
	}
	public void setIntValue(int nValue) {setValue(nValue);}
	public void setBoolValue(boolean bValue) {setValue(bValue ? 1.0 : 0.0);}

	public void setValue(int iParam, double fValue) {
            m_values[iParam] = fValue;
            m_bIsDirty = State.IS_DIRTY;
    }
	public void setIntValue(int iParam, int nValue) {setValue(iParam, nValue);}
	public void setBoolValue(int iParam, boolean bValue) {setValue(iParam, bValue ? 1.0 : 0.0);}


	/** keeping track of tidyness **/
    public void makeClean() {m_bIsDirty = State.IS_CLEAN;}
    public void makeDirty(int nDirt) {m_bIsDirty = nDirt;}
