...
public class ScaleOperator extends Operator {
	...
	double getLower() {return m_fLower;}
	double getUpper() {return m_fUpper;}
	...
	public double proposal(State state) throws Exception {
        ...
        if (m_pTree.get() != null) {
        	...
        	state.m_trees[m_nTreeID].getRoot().scale(scale);
        ...
        if (m_iVar < 0) {
        	m_iVar = state.getParameterIndex(m_pParameter.get().getID());
        }

        Parameter param = (Parameter) state.getParameter(m_iVar);
